...
export const getMovies = (args) => {
  ...
  return fetch(
    ...
    }&language=en-US&include_adult=false&include_video=false&page=${id}`
  )
    .then((response) => {
      if (!response.ok) {
...
export const getUpCommingMovies = (args) => {
  ...
  return fetch(
    `https://api.themoviedb.org/3/movie/upcoming?api_key=${
      import.meta.env.VITE_TMDB_KEY
    }&language=en-US&include_adult=false&page=${id}&sort_by=primary_release_date.desc`
  )
    .then((response) => {
      if (!response.ok) {
        throw new Error(response.json().message);
...
export const getTv = (args) => {
  ...
  const [, idPart] = args.queryKey;
  const { id } = idPart;
  const myURL =  `https://api.themoviedb.org/3/tv/${id}?api_key=${
    import.meta.env.VITE_TMDB_KEY
  }`;
  return fetch(
    myURL
